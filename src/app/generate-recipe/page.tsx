...
// // import { BeakerIcon, SparklesIcon, PlusCircleIcon, TrashIcon, ArrowPathIcon, ChevronDownIcon, ChevronUpIcon } from "@heroicons/react/24/outline";
...
// //   const [showRawOutput, setShowRawOutput] = useState(false); // For toggling raw output
...
// //                     <input type="text" value={ingredient} onChange={(e) => handleIngredientChange(index, e.target.value)} placeholder={`Ingredient ${index + 1}`} className="block w-full shadow-sm py-3 px-4 placeholder-gray-400 focus:ring-orange-500 focus:border-orange-500 border-gray-300 rounded-md"/>
// //                     {ingredients.length > 1 && <button type="button" onClick={() => handleRemoveIngredient(index)} className="text-red-500 hover:text-red-700 p-2 rounded-md hover:bg-red-50"><TrashIcon className="h-5 w-5" /></button>}
// //                   </div>
// //                 ))}
// //                 <button type="button" onClick={handleAddIngredient} className="mt-2 inline-flex items-center px-4 py-2 border border-dashed border-gray-300 text-sm font-medium rounded-md text-slate-700 bg-white hover:bg-gray-50"><PlusCircleIcon className="h-5 w-5 mr-2 text-gray-400" />Add Ingredient</button>
// //               </div>
...
// //                     <label key={diet} className="flex items-center space-x-2.5 cursor-pointer group"><input type="checkbox" value={diet} onChange={handleDietaryChange} className="h-4 w-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500 focus:ring-offset-1"/><span className="text-slate-700 group-hover:text-orange-600 transition-colors text-sm">{diet}</span></label>
// //                   ))}
// //                 </div>
// //               </div>
// //               {/* Cuisine, Skill, Meal Type */}
// //               <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
// //                 <div><label htmlFor="cuisine" className="block text-lg font-semibold text-slate-700 mb-2">Preferred Cuisine (Optional):</label><input type="text" id="cuisine" value={cuisine} onChange={(e) => setCuisine(e.target.value)} placeholder="e.g., Italian, Mexican" className="block w-full shadow-sm py-3 px-4 placeholder-gray-400 focus:ring-orange-500 focus:border-orange-500 border-gray-300 rounded-md"/></div>
// //                 <div><label htmlFor="skillLevel" className="block text-lg font-semibold text-slate-700 mb-2">Cooking Skill Level:</label><select id="skillLevel" value={skillLevel} onChange={(e) => setSkillLevel(e.target.value)} className="block w-full shadow-sm py-3 px-4 focus:ring-orange-500 focus:border-orange-500 border-gray-300 rounded-md bg-white"><option>Beginner</option><option>Intermediate</option><option>Advanced</option></select></div>
// //                 <div><label htmlFor="mealType" className="block text-lg font-semibold text-slate-700 mb-2">Meal Type (Optional):</label><select id="mealType" value={mealType} onChange={(e) => setMealType(e.target.value)} className="block w-full shadow-sm py-3 px-4 focus:ring-orange-500 focus:border-orange-500 border-gray-300 rounded-md bg-white"><option value="">Any</option><option>Breakfast</option><option>Brunch</option><option>Lunch</option><option>Dinner</option><option>Snack</option><option>Dessert</option><option>Appetizer</option><option>Side Dish</option></select></div>
// //               </div>
// //               {/* Specific Requests */}
...
// //                 <h2 className="text-3xl font-extrabold text-slate-800 sm:text-4xl">{generatedRecipe.name || "Your AI Generated Recipe"}</h2>
// //                 {generatedRecipe.description && <p className="mt-3 text-md text-slate-600 max-w-2xl mx-auto">{generatedRecipe.description}</p>}
// //             </div>
...
// //                         <>
// //                             <h3 className="text-xl font-semibold text-slate-700 mb-4">Ingredients:</h3>
// //                             <ul className="space-y-2 text-slate-700">
// //                                 {generatedRecipe.ingredients.map((ing, index) => (
// //                                     <li key={index} className="flex items-start">
// //                                         <span className="text-orange-500 mr-2 mt-1">✓</span>
// //                                         <span>{`${ing.quantity || ''} ${ing.unit || ''} ${ing.item}`.trim()}{ing.notes ? <span className="text-xs text-slate-500 italic"> ({ing.notes})</span> : ''}</span>
// //                                     </li>
// //                                 ))}
// //                             </ul>
// //                         </>
...
// //                         <>
// //                             <h3 className="text-xl font-semibold text-slate-700 mb-4">Instructions:</h3>
// //                             <ol className="space-y-3 text-slate-700 leading-relaxed">
// //                                 {generatedRecipe.instructions.map((step, index) => (
// //                                     <li key={index} className="flex">
// //                                         <span className="bg-orange-500 text-white rounded-full h-6 w-6 min-w-[24px] text-sm flex items-center justify-center mr-3 font-semibold">{index + 1}</span>
// //                                         <span>{step}</span>
// //                                     </li>
// //                                 ))}
// //                             </ol>
...
// //                     <h4 className="font-semibold text-sky-700 mb-1 text-lg">Chef&apos;s Notes:</h4>
// //                     <p className="text-sm text-sky-800 whitespace-pre-line">{generatedRecipe.notes}</p>
// //                 </div>
// //             )}
...
// //                     </Link>
// //                 ) : (
// //                      <button disabled className="w-full sm:w-auto order-1 sm:order-2 inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-slate-400 cursor-not-allowed">
// //                         Save Recipe (TODO)
// //                     </button>
// //                 )}
// //                  <button 
...
// //             <div className="mt-8">
// //                 <button onClick={() => setShowRawOutput(!showRawOutput)} className="text-xs text-slate-500 hover:text-slate-700 flex items-center">
// //                     {showRawOutput ? <ChevronUpIcon className="h-4 w-4 mr-1" /> : <ChevronDownIcon className="h-4 w-4 mr-1" />}
// //                     {showRawOutput ? "Hide" : "Show"} Raw AI Output (for debugging)
// //                 </button>
// //                 {showRawOutput && rawAIResponse && (
...
// // src/app/generate-recipe/page.tsx (Continued)
// // ... (imports and component setup from previous snippet) ...
...
//                 Tell us your preferences, and let our AI chef inspire you!
//               </p>
//             </div>
//             <form onSubmit={handleSubmit} className="bg-white shadow-xl rounded-lg p-6 sm:p-10 space-y-8">
//               {/* Ingredients Section */}
...
//               {/* Cuisine, Skill Level, Meal Type Section */}
//               <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-8">
//                 <div>
...
//             <button 
//                 onClick={handleTryAgain}
//                 className="inline-flex items-center px-6 py-2.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
//             >
//                 <ArrowPathIcon className="h-5 w-5 mr-2"/> Try Generating Again
//             </button>
//           </div>
//         )}
...
//             <div className="mt-10 pt-6 border-t border-slate-200 flex flex-col sm:flex-row gap-3 justify-center sm:justify-end">
//                 {generatedRecipe._id ? (
//                     <Link href={`/dashboard/my-recipes/${generatedRecipe._id.toString()}`} className="w-full sm:w-auto order-1 sm:order-2 inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
//                         View Saved Recipe
//                     </Link>
//                 ) : (
//                      <button disabled className="w-full sm:w-auto order-1 sm:order-2 inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-slate-400 cursor-not-allowed">
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (recipeIsPoorlyParsed) {
        setShowRawOutputToggle(true); // Default to showing raw if parsing seems off
      }
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-12 sm:py-16">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Show Form if no recipe is generated/loading and no error */}
        {(!generatedRecipe && !isLoading && !error) && (
          <>
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow-xl rounded-lg p-6 sm:p-10 space-y-8">
              {/* ... (Full Form JSX as before - Ingredients, Dietary, Cuisine, Skill, Meal Type, Specific Requests, Submit Button) ... */}
        ...
        {/* Generated Recipe Display Section (shown if generatedRecipe is not null and no error) */}
        {generatedRecipe && !error && !isLoading && (
          <div className="mt-12 bg-white shadow-xl rounded-lg p-6 sm:p-10">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-extrabold text-slate-800 sm:text-4xl">
                    {/* Use actual recipe name if parsed, otherwise the pending message or a default */}
                ...
                {/* Show description only if properly parsed AND recipe is considered parsed */}
            ...
            {isRecipeConsideredParsed ? (
                <>
                    {/* Structured Recipe Details (Prep time, Cook time, etc.) */}
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-4 gap-x-2 mb-8 text-sm text-center border-y border-slate-200 py-4">
                        {generatedRecipe.prepTime && <div><p className="text-slate-500">Prep Time</p><p className="font-semibold text-slate-700">{generatedRecipe.prepTime}</p></div>}
                        {generatedRecipe.cookTime && <div><p className="text-slate-500">Cook Time</p><p className="font-semibold text-slate-700">{generatedRecipe.cookTime}</p></div>}
                        {generatedRecipe.servings && <div><p className="text-slate-500">Servings</p><p className="font-semibold text-slate-700">{generatedRecipe.servings}</p></div>}
                ...
                // If not considered fully parsed, show the raw output as the primary content
                <div className="py-4">
            ...
            {/* Action Buttons: View Saved, Generate Another */}
            <div className="mt-10 pt-6 border-t border-slate-200 flex flex-col sm:flex-row gap-3 justify-center sm:justify-end">
                {generatedRecipe._id ? (
                    <Link href={`/dashboard/my-recipes/${generatedRecipe._id.toString()}`} className="w-full sm:w-auto order-1 sm:order-2 inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
