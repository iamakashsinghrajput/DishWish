...
import { BeakerIcon, SparklesIcon, PlusCircleIcon, TrashIcon, ArrowPathIcon, ChevronDownIcon, ChevronUpIcon, CheckCircleIcon, PhotoIcon } from "@heroicons/react/24/outline";
...
export default function GenerateRecipePage() {
  ...
  const [ingredients, setIngredients] = useState<IngredientInput[]>([
    { id: Date.now(), text: '', imageUrl: null, isLoadingImage: false }
  ]);
  ...
  const handleAddIngredient = () => {
    setIngredients([...ingredients, { id: Date.now(), text: '', imageUrl: null, isLoadingImage: false }]);
  };

  const handleIngredientChange = (id: number, value: string) => {
    setIngredients(prevIngredients =>
      prevIngredients.map(ing => (ing.id === id ? { ...ing, text: value } : ing))
    );
  };
  ...
  const fetchImageForIngredient = async (id: number, query: string) => {
    if (!query.trim()) {
        setIngredients(prev => prev.map(ing => ing.id === id ? { ...ing, imageUrl: null, isLoadingImage: false } : ing));
        return;
    }

    setIngredients(prev => prev.map(ing => ing.id === id ? { ...ing, isLoadingImage: true } : ing));

    try {
        const accessKey = process.env.NEXT_PUBLIC_UNSPLASH_ACCESS_KEY;
        if (!accessKey) throw new Error("Unsplash API key is not configured.");
        
        const response = await fetch(`https://api.unsplash.com/search/photos?query=${encodeURIComponent(query)}&per_page=1&orientation=squarish&client_id=${accessKey}`);
        if (!response.ok) throw new Error(`Failed to fetch image. Status: ${response.status}`);

        const data = await response.json();
        const imageUrl = data.results[0]?.urls?.small || null;

        setIngredients(prev => prev.map(ing => ing.id === id ? { ...ing, imageUrl, isLoadingImage: false } : ing));
    } catch (err) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('/api/ai/generate-recipe', {
        ...
        body: JSON.stringify({
          ingredients: ingredients.map(ing => ing.text).filter(i => i.trim() !== ''),
  ...
  if (status === "loading") {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-slate-100">
        ...
        <p className="mt-4 text-xl font-semibold text-slate-700">Authenticating...</p>
      </div>
    );
  }
  ...
  if (status !== "authenticated" || !session) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-slate-100">
        <p className="text-xl font-semibold text-slate-700">Redirecting to login...</p>
         <p className="text-sm text-slate-500">You need to be logged in to generate recipes.</p>
      </div>
    );
  }
          ...
          <div className="mt-12 bg-white shadow-xl rounded-lg p-6 sm:p-10">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-extrabold text-slate-800 sm:text-4xl">
                    {generatedRecipe.name || "AI Generated Recipe"}
                </h2>
            ...
            {isRecipeConsideredParsed ? (
                        ...
                        {generatedRecipe.totalCalories && (
                            <div className="flex flex-col items-center justify-center">
                                <p className="text-slate-500 flex items-center">
                                    <FireIcon className="h-4 w-4 mr-1 text-red-400"/>
                                    Calories
                                </p>
                                <p className="font-semibold text-slate-700">
                                    ~{generatedRecipe.totalCalories.toLocaleString()} kcal
                                </p>
                            </div>
                        )}
                ...
                <div className="py-4">
                    <p className="text-center text-slate-600 mb-2">The AI is still perfecting the recipe details, or the format wasn&apos;t fully recognized.</p>
                    <p className="text-center text-slate-600 mb-6 font-semibold">Here&apos;s the direct output from the AI for you to review:</p>
                    {rawAIResponse ? (<pre className="text-sm bg-gray-50 p-4 rounded-md overflow-x-auto whitespace-pre-wrap border border-gray-200">{rawAIResponse}</pre>) : (<p className="text-center text-slate-500">No raw output available to display.</p>)}
                </div>
            )}
            <div className="mt-10 pt-6 border-t border-slate-200 flex flex-col sm:flex-row gap-3 justify-center sm:justify-end">
                {generatedRecipe._id ? (<Link href={`/dashboard/my-recipes/${generatedRecipe._id.toString()}`} className="w-full sm:w-auto order-1 sm:order-2 inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500"><CheckCircleIcon className="h-5 w-5 mr-2" /> View Saved Recipe</Link>) : (<button title="Save recipe" disabled className="w-full sm:w-auto order-1 sm:order-2 inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-slate-400 cursor-not-allowed">Save Recipe (TODO)</button>)}
                 <button onClick={handleTryAgainOrNew} className="w-full sm:w-auto order-2 sm:order-1 inline-flex items-center justify-center px-6 py-3 border border-slate-300 text-base font-medium rounded-md shadow-sm text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500"><SparklesIcon className="h-5 w-5 mr-2"/> Generate Another</button>
            </div>
            <div className="mt-8"><button onClick={() => setShowRawOutputToggle(!showRawOutputToggle)} className="text-xs text-slate-500 hover:text-slate-700 flex items-center">{showRawOutputToggle ? <ChevronUpIcon className="h-4 w-4 mr-1" /> : <ChevronDownIcon className="h-4 w-4 mr-1" />}{showRawOutputToggle ? "Hide" : "Show"} Full Raw AI Output</button>{showRawOutputToggle && rawAIResponse && (<pre className="mt-2 text-xs bg-gray-100 p-4 rounded-md overflow-x-auto whitespace-pre-wrap border border-gray-200">{rawAIResponse}</pre>)}{!rawAIResponse && showRawOutputToggle && <p className="text-xs text-slate-400 mt-1">No raw output data.</p>}</div>
          </div>
        ) : (
          <>
            ...
            <form onSubmit={handleSubmit} className="bg-white shadow-xl rounded-lg p-6 sm:p-10 space-y-8">
              <div>
                <label className="block text-lg font-semibold text-slate-700 mb-4">Ingredients You Have:</label>
                <div className="space-y-4">
                    {ingredients.map((ing) => (
                      <div key={ing.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-3">
                        <input
                          type="text"
                          value={ing.text}
                          onChange={(e) => handleIngredientChange(ing.id, e.target.value)}
                          onBlur={() => fetchImageForIngredient(ing.id, ing.text)}
                          placeholder={`e.g., 2 large eggs`}
                          className="block w-full shadow-sm py-3 px-4 placeholder-slate-400 focus:ring-orange-500 focus:border-orange-500 border-gray-300 rounded-md"
                        />
                        
                        <div className="w-16 h-16 flex-shrink-0 bg-slate-100 rounded-lg flex items-center justify-center overflow-hidden border border-slate-200">
                          {ing.isLoadingImage ? (
                            <div className="animate-spin rounded-full h-6 w-6 border-t-2 border-b-2 border-orange-500"></div>
                          ) : ing.imageUrl ? (
                            <Image
                              src={ing.imageUrl}
                              alt={ing.text || 'Ingredient'}
                              width={64}
                              height={64}
                        ...
                        {ingredients.length > 1 && (
                            <button type="button" onClick={() => handleRemoveIngredient(ing.id)} className="text-red-500 hover:text-red-700 p-2 rounded-md hover:bg-red-50 transition-colors">
                ...
                <button type="button" onClick={handleAddIngredient} className="mt-4 inline-flex items-center px-4 py-2 border border-dashed border-gray-300 text-sm font-medium rounded-md text-slate-700 bg-white hover:bg-gray-50"><PlusCircleIcon className="h-5 w-5 mr-2 text-gray-400" />Add Ingredient</button>
              </div>
              
              <div><label className="block text-lg font-semibold text-slate-700 mb-3">Dietary Restrictions (Optional):</label><div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3">{['Vegetarian', 'Vegan', 'Gluten-Free', 'Dairy-Free', 'Nut-Free', 'Low-Carb', 'Keto', 'Paleo', 'Jain'].map(diet => (<label key={diet} className="flex items-center space-x-2.5 cursor-pointer group"><input type="checkbox" value={diet} onChange={handleDietaryChange} className="h-4 w-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500 focus:ring-offset-1"/><span className="text-slate-700 group-hover:text-orange-600 transition-colors text-sm">{diet}</span></label>))}</div></div>
