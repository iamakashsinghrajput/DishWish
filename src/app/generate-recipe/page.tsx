...
export default function GenerateRecipePage() {
  ...
  // ... (rest of your state variables: ingredients, dietaryRestrictions, etc.)
  const [ingredients, setIngredients] = useState<string[]>(['']);
  const [dietaryRestrictions, setDietaryRestrictions] = useState<string[]>([]);
  const [cuisine, setCuisine] = useState('');
  const [skillLevel, setSkillLevel] = useState('Beginner');
  ...
  const [generatedRecipe, setGeneratedRecipe] = useState<Partial<IRecipe> | null>(null);
  const [rawAIResponse, setRawAIResponse] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false); // This is for AI generation loading
  const [error, setError] = useState<string | null>(null);
  const [showRawOutputToggle, setShowRawOutputToggle] = useState(false);


  useEffect(() => {
    // This effect handles redirection if the user becomes unauthenticated
    // or if the initial status (after loading) is unauthenticated.
    if (status === "unauthenticated") {
      console.log("GenerateRecipePage: Status is unauthenticated, redirecting to login.");
      router.replace("/session/new?callbackUrl=/generate-recipe");
    }
  }, [status, router]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      setIsLoading(false); // For AI call
    }
  };
  ...
  // If, after loading, status is still not "authenticated", the useEffect will trigger a redirect.
  // This is an additional safeguard or can display a message if redirect is slow.
  if (status !== "authenticated" || !session) {
    // The useEffect should handle the redirect. This is a fallback rendering.
    // In most cases, the redirect will happen before this is seen for long.
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-slate-100">
  ...
  // --- Render actual page content only if authenticated ---
  const isRecipeConsideredParsed = generatedRecipe && 
                                (generatedRecipe.name && generatedRecipe.name !== "AI Generated Recipe (Parsing Pending)" && generatedRecipe.name !== "Untitled AI Recipe") && 
                                (generatedRecipe.instructions && generatedRecipe.instructions.length > 0) &&
                                (generatedRecipe.ingredients && generatedRecipe.ingredients.length > 0);
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-12 sm:py-16">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Show Form or Loading for AI or Recipe/Error */}
        {isLoading ? ( // This `isLoading` is for the AI generation call
            <div className="mt-12 bg-white shadow-xl rounded-lg p-10 text-center">
                <div className="flex justify-center items-center mb-4">
                    <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-b-4 border-orange-500"></div>
                </div>
