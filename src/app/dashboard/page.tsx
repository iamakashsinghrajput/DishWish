...
export default function DashboardPage() {
  ...
  if (error) {
     return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-slate-100 text-center px-4">
        <p className="text-xl font-semibold text-red-600">Oops! Something went wrong</p>
  ...
  if (!dashboardData) {
      return (
          <div className="flex flex-col justify-center items-center min-h-screen bg-slate-100"><p className="text-xl">Welcome! Let&apos;s get your dashboard ready.</p></div>
      )
  }
              ...
              {recentRecipes.length > 0 ? (
                <ul className="space-y-4">
                  {recentRecipes.map((recipe) => (
                    <li key={recipe._id?.toString()} className="p-4 border border-slate-200 rounded-lg hover:shadow-md transition-shadow">
                      <Link href={`/dashboard/my-recipes/${recipe._id}`} className="block group">
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                          {recipe.cuisine && <span className="mr-2 capitalize">{recipe.cuisine}</span>}
                ...
                <div className="text-center py-10">
                    <p className="text-slate-500">No recent recipes to display.</p>
                    <Link href="/generate-recipe" className="mt-4 inline-block text-sm font-medium text-orange-600 hover:text-orange-500">
